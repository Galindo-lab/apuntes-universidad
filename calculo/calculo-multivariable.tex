...
\label{sec:orgd141e0b}
\subsection{Eje coordenado}
\label{sec:orgfe5ebdb}
...
\item El ángulo \(\theta\) (theta) \textbf{Si} ocupa \hyperref[tab:org926b25c]{corrección}.
\end{enumerate}
\end{itemize}
...
\label{sec:orgf1abe28}
\subsection{Vector Unitario}
\label{sec:orgef53f71}
...
sabemos que la función de raíz cuadrada existe para todos los valores que son igual ó mayor a '0' (en números reales), entonces el dominio de la función seria\footnote{Ambas formas de expresarlas son correctas.}:
...
\item \(x+y+1 \geq 0\), la suma de \(x+y+1\) es mayor ó igual (\(\ge\)) que 0\ldots{}
\item \(\land\), y\ldots{}
...
como los valores de 'x' y 'y' se están restando\footnote{Al ser cuadrados los valores que obtenemos siempre son positivos.} y la raiz no puede ser menor a '0' entonces determinamos que el \uline{maximo valor maximo} de \(9-x^2-y^2\) es '9' y como la funcion es raiz \(\sqrt{9}=3\).

Ahora para el \uline{valor minimo} no hay mucho misterio, la funcion es raiz y en los reales no hay raices negativas, entonces el minimo valor es 0. Entonces el rango quedaria:
...
\label{sec:orgc70847a}
\subsection{Calculadoras}
\label{sec:org0e8547e}
\begin{itemize}
...
\item \href{https://www.wolframalpha.com/widgets/view.jsp?id=537159240ca725f712f6c7437d2f3e0c}{Maximos y minimos}
\end{itemize}

\subsection{Paginas}
\label{sec:org4d82cfc}
\begin{itemize}
...
\item \href{https://blog.unitips.mx/dominio-y-rango-de-funciones}{Rango de una funcion}
\end{itemize}

\subsection{Videos}
\label{sec:org83daf2f}
\begin{itemize}
