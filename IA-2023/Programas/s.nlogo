...
to setup
  ...
  set falure-rates array:from-list (list
    failure-rate-1
    failure-rate-2
    failure-rate-3
    failure-rate-4
  ...
  set operators-station array:from-list (list
    operators-1
    operators-2
    operators-3
    operators-4
  ...
  set stations-delay-rate array:from-list (list
    complexity-rate-1
    complexity-rate-2
    complexity-rate-3
    complexity-rate-4
  ...
  set stations-complexity-rate array:from-list (list
    complexity-rate-1
    complexity-rate-2
    complexity-rate-3
    complexity-rate-4
...
to-report calc-dexterity
  let variance-dex (random variance-dexterity)

  (ifelse
    fix-dexterity [ report fixed-dexterity ]
    [ report (in-range-random (- variance-dex) variance-dex) + base-dexterity ])
end

to-report in-range-random [minnumber maxnumber]
...
to go
  ask operator [
    ...
    let n-object-stage (array:item objects-stage station)

    if n-object-stage > 0 [

      ; verificar si se pudo ensamblar la pieza
      let completion-probability random 100
      let station-completion array:item stations-delay-rate station
      let station-complexity array:item stations-complexity-rate station

      (ifelse
        (completion-probability * (dexterity / 100)) > station-completion [
          set color green
        ...
        [ ; else
          ...
          if (random 100) < array:item falure-rates station [
            ; si se atraza verificar la probabilidad de que se daño el objeto
            let obj-current-station array:item objects-stage station
            array:set objects-stage station (obj-current-station - 1)
  ...
  ifelse
  ((array:item objects-stage n-stations) + damanged = n-items) [
